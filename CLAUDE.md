...
- **(later same day) GST provider switched to gstincheck.co.in (user decision, supersedes the Cashfree plan):** `GstinCheckProvider` (`GET sheet.gstincheck.co.in/check/{key}/{gstin}`) behind the same seam; `GST_PROVIDER=gstincheck` + `GSTINCHECK_API_KEY` (set in `.env.local`; MUST also be set in Vercel env to go live in prod). Error policy verified during a REAL GSTN outage: only invalid/not-found GSTINs reject; upstream/key/credit errors throw → provisional registration. 10-min same-instance cache halves the double-verify credit burn (20 free credits on the account). §3 stack table's "Surepass" plan is superseded by this entry.
...
### 2026-06-13/14 — Go-live (domain + email) + three user-directed UX/feature batches (#1–37)

**Production topology (external config, NOT in the repo — record so future-me doesn't hunt):** App is live at **https://app.chemibid.com** (Vercel custom domain — GoDaddy `CNAME app → …vercel-dns…`, SSL valid) plus the default `chem-bid-platform.vercel.app`. The Vercel project lives under the **chemibid-7256** account (the user's personal `vrajtalatii@gmail.com` Vercel 404s on it). Email sends from **notify@chemibid.com** (Resend; chemibid.com domain verified — non-owner delivery works). Vercel env added this session: `NEXT_PUBLIC_APP_URL=https://app.chemibid.com`, `EMAIL_FROM`. Supabase Auth → URL Configuration set: Site URL = `https://app.chemibid.com`, Redirect URLs = `https://app.chemibid.com/**` + `http://localhost:3000/**` (was localhost-only + empty — a latent prod bug).

**Push/deploy constraint (recurring — bit me repeatedly):** every `git push` MUST run as the **vrajtalati** gh account → run `gh auth switch --hostname github.com --user vrajtalati` immediately before EACH push (it silently reverts to `hellotwoclicksmedia-ship-it`, which 403s). Push to `master` auto-deploys; confirm via `gh api repos/chemibid-cyber/Chem-Bid-Platform/commits/<sha>/status` (Vercel posts the state there).

**Password reset + member invite — FIXED (token_hash pattern).** Both used `admin.generateLink` → Supabase `/auth/v1/verify` action_link → `/auth/callback` `exchangeCodeForSession` (PKCE), which can't complete for admin-generated links (no browser code_verifier) AND the single-use verify URL was burned by inbox link-scanners (`otp_expired`). Now: email a link to OUR OWN page carrying `linkData.properties.hashed_token` — reset → `/reset-password?token_hash=…&type=recovery`; invite → `/accept-invite?token_hash=…&type=invite`. The page only renders a form on GET (no token consumption); the SUBMIT action calls `supabase.auth.verifyOtp({ token_hash, type })` (scanners don't POST), then updateUser / activate. `/accept-invite` no longer needs a pre-session. Verified live: `scripts/test-reset.ts`, `scripts/test-invite.ts`. NOTE: stale pre-fix emails still fail — request a fresh link.

**OTP contact verification (email LIVE; phone test-mode).** Settings → "Verify contact details": 6-digit OTP to email (Resend) or phone. New `otp_codes` table (service-role RLS, default-deny + `revoke`), codes HMAC-peppered (`OTP_HMAC_SECRET` ?? `CRON_SECRET` fallback so prod works with no new env var), single-use, 10-min TTL, 5-attempt cap, DB-enforced send caps (per-user / per-destination / global SMS kill-switch — the in-memory limiter can't gate cost across Vercel lambdas), atomic TOCTOU-safe verify, constant-time compare, generic errors. SMS behind a `SmsOtpProvider` seam (`src/lib/sms/`): MockSmsProvider (dev; throws in prod) + Msg91Provider. **Phone is hidden in the UI until `SMS_PROVIDER != mock`** — India DLT (TRAI) registration is required to actually deliver OTP SMS; tested seven.io via RapidAPI = does NOT send (returns balance, no charge); Twilio/MSG91 also require DLT; only Firebase Phone Auth / WhatsApp avoid it. Migration `0004`.

**Batch #1–13 (UX fixes):** profile dropdown closes on select (new `ProfileMenu`); login password eye toggle (new `PasswordInput`); Terms "Back" = `router.back()` (was hardcoded `/`); New-Auction Enter no longer submits; auction chemical via catalog dropdown (purchase profile, auto-fill + lock CAS/name, "Other" fallback to manual resolve); nav highlights only the most-specific item; "Company & profile" → "My Company Profile"; drag-drop `FileDropzone` (auction spec / COA / dispute); double-submit audited (shared `SubmitButton`/`ConfirmButton` already safe); member Can-buy/Can-sell default UNCHECKED.

**Batch #14–24 (migration `0005`):** paymentTermsEnum expanded (+immediate/net7/net60/net90/net120/other); new `freightTermsEnum`, `counterProposalStatusEnum`; logisticsBasis +`other`. auctions: paymentTerms (+custom), deliveryTermsCustom, freightTerms, offer/supplyValidUntil. bids: `stage1TaxPct` (tax entered as a % → auto-calc amount; ranking still on the full total). blocks: `expiresAt`. New **`counter_proposals`** table. Features: delivery "Other"+custom; ignore→`/requests`; block scope (this CAS/all) + duration (1/3/6mo/permanent, expiry enforced in targeting); `/network/blocked` page + unblock; auction payment terms shown to sellers; freight = 0 now allowed (validateBidPricing relaxed); offer/supply validity pickers; **buyer contact (email/phone/location) revealed to sellers POST-ACCEPT only** (blind rule preserved); **#21 seller structured counter-proposal** — propose qty/packing/delivery/validity → buyer accepts/rejects; single-winner + full-quantity preserved, base auction spec NEVER auto-rewritten on accept.

**Batch #25–37:** "Revise bid"→"Modify bid"; explicit closing date+time on bid screen + listings; **`DecimalInput`** (2dp clamp, scientific-notation-safe) on all money/qty inputs + server `round2`; self-match already excluded in `targeting/run.ts` (`ne(companyId, buyerCompanyId)` both directions — verified, no change); capability checkbox visibility/alignment fixed (kept native `accent-brand` — an `appearance-none` + data-URL-SVG attempt BROKE the webpack CSS build, reverted); member **Re-enable** + **Delete** (Delete hard-removes ONLY a member with zero catalog/auction/bid — e.g. a never-accepted invite — to protect FKs + the append-only audit; anyone with activity is steered to Disable+reassign); "Become a provider" card hidden from buyer/seller service views (kept for provider-only accounts); invite **Resend** + pending/active status + **Edit-pending** (email change updates the Supabase auth user + `users.email` + re-sends a fresh token); Admin/Buyer/Seller header indicator; **≥1 capability required** at member creation (client + server); reset expired-link error + "request a new link" CTA; password eye toggle on signup/reset/accept-invite; **polling toast notifications** (`src/components/ui/toaster.tsx` + `components/app/notification-toaster.tsx` + `notifications/poll-actions.ts` — visibility-gated, deduped, baseline = "now" so history never replays; chose polling over Supabase realtime to avoid publication/RLS config).

**Orchestration discipline (how these batches were built):** parallel subagents partitioned by DISJOINT file ownership, with the schema + shared constants (`format.ts` label/option maps) laid down FIRST as the contract; cross-cutting/spanning features (e.g. #21) built sequentially on the stable base; then adversarial reviewers (privacy / pricing / blocks / counter / member-delete FK-safety / toast RSC boundary) before each deploy. New helper scripts under `scripts/`: `test-reset`, `test-otp`, `test-invite`, `send-test-invite`, `print-reset-link`, `apply-sql`, `check-invite`. Migrations 0004 + 0005 applied to live Supabase via `npm run db:migrate`. As of 2026-06-14: typecheck + build clean (35 routes), **78 unit tests** green.

### 2026-06-17 — Member-level data isolation (#39–42) + Clarity/Tidio widgets

**Member-level isolation (#40/#41/#42 — commit `fbaff20`).** Within a company, a non-admin member now sees and acts on ONLY the records they own; admins keep the full company-wide view. Company id stays the security boundary (RLS unchanged) — this is a within-company UX/authz layer at the app level. New `src/lib/auth/scope.ts`: `ownerScope(column, user)` (returns `eq(col,user.id)` for members, `undefined` for admins → drops into `and(...)` so admins are unrestricted) for list filters, and `canAccessOwned(ownerUserId, user)` (admin || own) for detail/action gates. Owner columns: `auctions.buyerUserId`, `bids.sellerUserId`, `catalogItems.ownerUserId`, `serviceRequests.neederUserId`. **The adversarial review caught that the first pass scoped READS but not WRITES** — every mutating server action was company-only, so a member could cancel/confirm-deal/withdraw a colleague's auction/bid by id. Fixed: gated all mutating actions in auctions/requests/catalog/services after load (extend/cancel/launchStage2/confirmDeal/blockSeller/respondToCounter, submit/accept/ignore/withdraw bid, stage2Respond, counter-propose, delist, cancel/accept service quote); `acceptRequestAction` no longer reassigns `sellerUserId` to the caller (admin acting can't steal ownership); the auction **export route** gates the buyer side on `buyerUserId` (also closed a pre-existing gap). Provider marketplace + targeting stay company-wide by design; `deals` stays company-scoped (no owner-user column). **#39:** publishing a service request blocks an identical OPEN request from the same member within 5 min (null-safe `materialSpec`) before insert.
